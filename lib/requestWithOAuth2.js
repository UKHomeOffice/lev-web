...
function get(url, oauthUrl, clientId, clientSecret, username, password, callback) {

  function useOAuth(err, res, body) {
    try {
      accessToken = JSON.parse(body).access_token;
      doRequestWithAccessToken(url, accessToken, callback);
    } catch(err) {
      callback(err, res, body);
    }
  }

  function requestURLWithAccessToken(url, accessToken, oauthUrl, clientId, clientSecret, username, password, callback) {
    // Try request with current access token
    doRequestWithAccessToken(url, accessToken, function checkResponse(err, res, body) {
  ...
  if (_.isUndefined(oauthUrl) || _.isUndefined(clientId) || _.isUndefined(clientSecret) ||
    ...
    if (_.isUndefined(accessToken)) {
      ...
      requestURLWithAccessToken(url, accessToken, oauthUrl, clientId, clientSecret, username, password, callback);
    }
  }
}
...
function doRequestWithAccessToken(url, accessToken, callback) {
